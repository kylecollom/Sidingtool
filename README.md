...
| **Openings perimeter vs. "new wraps"** | Originally the openings-perimeter field (and its contribution to J-Channel / HardieTrim 4" opening trim) was gated behind the "new window/door wraps?" toggle — wrong, since J-channel/trim is what terminates the siding at each opening and is needed regardless of whether the customer is also paying for new decorative wraps. Fixed: the field is now always shown and always counted. The toggle itself no longer drives any calculation — no wrap-specific material is modeled yet — so it now just adds a site note flagging that wrap material needs to be priced separately | Windows & Doors section of `JobForm.tsx` / `calculate.ts` |
